...
def sup_collate_fn(batch):
    ...
    token_type_ids = torch.tensor(
        [x['token_type_ids'][:max_len] for x in batch])
...
def unsup_collate_fn(batch):
    ...
    ori_token_type_ids = torch.tensor(
        [x['ori_token_type_ids'][:max_len] for x in batch], dtype=torch.long)
    aug_token_type_ids = torch.tensor(
        [x['aug_token_type_ids'][:max_len] for x in batch], dtype=torch.long)
    ...
    all_token_type_ids = torch.cat(
        [ori_token_type_ids, aug_token_type_ids], dim=0)
